...
module.exports = async () => {
    ...
    page.on('request', request => {
        if (
            request.url.indexOf('https://www.icemachinesplus.io') === -1 || ['Stylesheet', 'Image', 'Media', 'Font'].includes(request.resourceType) ||
            (/\.(png|jpg|jpeg|gif|svg|webp|css|woff2|bmp|mp4|eot|woff|ttf)$/i).test(request.url)
        )
            request.abort();
        else
            request.continue();
